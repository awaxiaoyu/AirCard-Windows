...
use crate::apple::{get_apple_libraries, ATHostConnectionRef};
use crate::device::{ensure_transport_available, DeviceTransport};
...
fn plist_to_cf(
    libs: &crate::apple::AppleLibraries,
    value: &plist::Value,
) -> Result<crate::apple::CFTypeGuard> {
    ...
    L: FnMut(&str),
{
    ...
    let mut conn: ATHostConnectionRef = unsafe {
        // This overload initializes the library context used by Apple's Grappa handshake.
        (libs.at_host_connection_create_with_library)(cf_library.raw, cf_udid.raw, 0)
    };
    if conn.is_null() {
        log("CreateWithLibrary returned null; falling back to ATHostConnectionCreate...");
        conn = unsafe { (libs.at_host_connection_create)(cf_udid.raw, std::ptr::null()) };
    }
    if conn.is_null() {
    ...
    let mut run_sync = || -> Result<()> {
        ...
        log(&format!(
            "AirTraffic session {session}, sending HostInfo..."
        ));
